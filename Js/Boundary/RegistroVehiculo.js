...
marc.addEventListener('eventofiltrar', e => {
    ...
    controlador.find(`marca/search?nombre=${filtro}`).
    then(data => {
        let lista=[];
        let ide=[];
        if(data!==null){
...
div.addEventListener('eventofiltrar', e => {
    let filtro = e.detail.filtro;
    controlador.find(`${pathModel}?modelo=${filtro}`).
    then(data => {
        let lista=[];
        let id=[];
        if(data!==null){
...
pro.addEventListener('eventofiltrar', e => {
    let filtro = e.detail.filtro;
    controlador.find(`propietario/search?dui=${filtro}`).
    then(data => {
        let lista=[];
        if(data!==null){
            data.forEach(dato=>{
...
tpv.addEventListener('eventofiltrar',e=>{
    let filtro = e.detail.filtro;
    controlador.find(`tipovehiculo/search?nombre=${filtro}`).
    then(data => {
        let lista=[];
        let ide=[];
        if(data!==null){
...
completeMarca.addEventListener('eventofiltrar', e => {
    let filtro = e.detail.filtro;
    controlador.find(`marca?nombre=${filtro}`).
    then(data => {
        let lista=[];
        let ide=[];
        if(data!==null){
